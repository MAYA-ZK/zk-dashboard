import { polygonZkEvmBlocks } from '@zk-dashboard/common/database/schema'
import { db } from '@zk-dashboard/common/database/utils'
import type { PolygonZkEvmRpcBlock } from '@zk-dashboard/common/integrations/polygon-zk-evm/rpc'
...
async function insertBlocks(blocksInput: Array<PolygonZkEvmRpcBlock>) {
  logger.info(
    LOGGER_TAG,
  ...
  const values = blocksInput.map((block) => ({
    ...
    // polygonZkEvm has some blocks without transactions (e.g. block number 19567604)
